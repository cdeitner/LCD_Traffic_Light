...
static void LCD_GPIOInit(void){
    ...
    wait++;                                     // wait for port activation
    GPIO_PORTD_DIR_R = 0x04;                    // make PD2 an output
    ...
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTQ_DIR_R = 0x0D;                    // make PQ0, PQ2-3 outputs
    GPIO_PORTQ_AFSEL_R = 0x00;                   // disable alternate functions
    GPIO_PORTQ_DEN_R = 0x0D;                    // enable digital I/O on PQ0, PQ2-3
}
...
void LCD_PrintFloat(float num){
    unsigned char i = 0;
    unsigned char sign = ' ';
    unsigned char tempString[30];

    // If our number is 0, print 0
    if (num == 0) {
        LCD_PrintString("0.00");
        return;
    }
